...
function BoothPage({
  ...
  const activeCaptureAspectRatio = useMemo(() => {
    const slots = Array.isArray(outputFrame.photoSlots) ? outputFrame.photoSlots : []
    const slotIndex = Math.max(0, currentPhotoIndex - 1)
    ...
    if (
      ...
      Number.isFinite(slotHeightPercent) &&
      slotWidthPercent > 0 &&
      slotHeightPercent > 0
    ) {
      return (slotWidthPercent * canvasAspectRatio) / slotHeightPercent
    }
  ...
  const cameraPreviewStyle = {
    ...
    ...(isFocusCameraMode
      ? {
          maxWidth: `${Math.min(activeCaptureAspectRatio * 92, 96)}vh`,
        }
      : {}),
  }
  ...
  const handleStartSession = useCallback(async () => {
    ...
    for (let index = 1; index <= effectiveTotalPhotos; index += 1) {
      ...
      const photo = captureVideoFrame(videoRef.current, capturePreviewRef.current, { aspectRatio: activeCaptureAspectRatio })
      ...
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-[1500px] flex-col gap-4 xl:h-full xl:min-h-0">
        ...
        <section className="grid flex-1 gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(260px,0.65fr)_minmax(280px,0.75fr)] xl:min-h-0">
          <div
            className={
              isFocusCameraMode
                  ? 'fixed inset-0 z-50 flex items-center justify-center bg-slate-950 p-4'
                  : 'relative min-h-0'
            }
          >
              <CameraPreview
                videoRef={videoRef}
                containerRef={isFocusCameraMode ? capturePreviewRef : null}
                isCameraReady={isCameraReady}
                errorMessage={cameraError}
                className={
                  isFocusCameraMode
                    ? 'relative w-[calc(100vw-2rem)] max-h-[calc(100vh-2rem)] overflow-hidden rounded-[1.8rem] border border-white/10 bg-slate-900 shadow-2xl shadow-black/30'
                    : 'relative w-full overflow-hidden rounded-[1.8rem] border border-white/10 bg-slate-900 shadow-2xl shadow-black/30'
                }
              ...
              {isShutterFlashing && (
                <div
                  className="pointer-events-none absolute inset-0 z-40 bg-white/90"
                  aria-hidden="true"
                />
              )}
